...
function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || json.code !== 0 || !json.data?.accessToken) {
        const parts = [json.message, json.hint].filter(
          (x): x is string => Boolean(x && String(x).trim()),
        );
        setError(parts.length ? parts.join("\n") : "账号或密码错误");
        return;
      }
      localStorage.setItem(ADMIN_TOKEN_STORAGE_KEY, json.data.accessToken);
  ...
  return (
    <div className="grid min-h-dvh lg:grid-cols-2">
      ...
      {/* Right Login Section */}
      <div className="flex items-center justify-center bg-background px-4 py-8 pb-[max(2rem,env(safe-area-inset-bottom))] sm:p-8">
        <div className="w-full max-w-[420px]">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute right-2 top-1/2 flex size-11 -translate-y-1/2 items-center justify-center text-muted-foreground touch-manipulation hover:text-foreground sm:right-3 sm:size-9"
                >
                  {showPassword ? (
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-muted-foreground">忘记密码？（预留）</span>
            </div>
