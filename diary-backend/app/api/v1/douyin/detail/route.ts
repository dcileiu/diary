...
import {
  DOUYIN_VIDEO_PREFERENCES,
  getDouyinDetail,
  type DouyinDetailParams,
...
function asVideoPreference(value: string | undefined): DouyinVideoPreference {
  if (!value) return "resolution";
  return (DOUYIN_VIDEO_PREFERENCES as readonly string[]).includes(value)
    ? (value as DouyinVideoPreference)
    : "resolution";
}
...
export async function POST(req: Request) {
  try {
    ...
    const params: DouyinDetailParams = {
      url,
    ...
    const result = await getDouyinDetail(params);
    return mpOk(result);
  } catch (e) {
    console.error("[douyin/detail]", e);
